...
Исходники дизайн-канваса с концепциями заставки. Десять направлений
...
Артборды генерируются скриптами `gen1.py`…`gen5.py` (общие куски —
знак, шрифты, обёртка страницы — в `_common.py`; `gen5.py` переиспользует
