  /* ============ מסך הפתיחה: גלגל שנופל, מקפיץ, ומתגלגל ============
   ...
   הגלגל שוקע לתוכה, היא נמתחת ומתקשה כמו יריעה אמיתית, וזורקת
   אותו בחזרה גבוה. אחר כך הוא נוחת על הרצפה ומקפץ עוד כמה
   קפיצות דועכות — כמו גלגל אמיתי — ורק אז מתיישב לגלגול.
...
(function () {
  ...
  var ZETA = 0.22;         // בולם הצמיג — ממנו נולדת ההקפצה הדועכת
  ...
  /* הסימן מתנהג כיריעת טרמפולינה: רך בהרבה מהצמיג, ולכן הוא זה
     שנמתח — הגלגל שוקע לתוכו ונזרק בחזרה. כמעט בלי בליעה, ולכן
     רוב האנרגיה חוזרת. היריעה מתקשה ככל שהיא נמתחת, בדיוק כמו
     טרמפולינה אמיתית, וזה מה שמגביל את השקיעה. */
  var ML = 1.2, KL = 2400, CL = 23;
  ...
  function sub(h) {
    ...
    if (pen > 0) {
      ...
      var q = pen / (BOTTOM * R);
      Fn = K * pen * (1 + 20 * q * q * q * q) - C * vn;       // מתקשה על החישוק
      if (Fn < 0) Fn = 0;
    ...
    // היריעה: קפיץ מתקשה, כמעט בלי בליעה
    var u = md / (BED * markH);
