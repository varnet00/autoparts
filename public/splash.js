  /* ============ מסך הפתיחה: גלגל שנופל, מקפיץ, ומתגלגל ============
   ...
   הגלגל נופל בלי להסתובב כלל, כמו ברנדר — נמדד שם על טבעת הכיתוב
   שבדופן: אפס מעלות לאורך כל הנפילה. הסיבוב והנסיעה נולדים רק
   מהפגיעה: כתר הסימן מעוגל, ולכן הנורמל במקום הנחיתה נוטה הצידה,
   והחיכוך בפתח המגע הופך את הבעיטה הזו גם לגלגול. אין שום דחיפה
   כתובה ביד.
...
(function () {
  ...
  var BASE_SLOPE = 0.20;   // שיפוע כתר הסימן על מסך צר
  var CROWN_SLOPE;         // נגזר מאורך המסלול
  ...
  /* על מסך רחב יש יותר דרך לעבור, ולכן הנחיתה על הכתר תלולה יותר
     והבעיטה חזקה יותר. זה פרמטר של הסצנה ולא שינוי בדינמיקה —
     אחרת על מסך רחב הגלגל היה מתגלגל בנחת עוד שניות אחרי שהכול
     כבר עמד במקום. */
  var runway = exitX + R - startX;
  CROWN_SLOPE = Math.max(BASE_SLOPE, Math.min(4.5 * BASE_SLOPE, BASE_SLOPE * Math.pow(runway / 345, 1.5)));
  ...
  var vx = 0, vy = 0, rot = 0, om = 0;     // נופל בלי סיבוב, כמו ברנדר
