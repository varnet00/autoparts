  /* ============ מסך הפתיחה: גלגל שנופל, מקפיץ, ומתגלגל ============
   ...
   הסימן הוא טרמפולינה: יריעה רכה בהרבה מהצמיג, ולכן היא זו שנמתחת.
   הבליעה שלה אסימטרית — פנימה כמעט חופשית, החוצה בולעת — ולכן היא
   נמתחת עמוק לעין וזורקת בחזרה נמרץ אבל לא מחוץ למסך.
   אחר כך הגלגל נוחת על הרצפה ומקפץ כמו כדור: שמונה קפיצות
   שהולכות וקטנות, ורק אז הוא מתיישב לגלגול. זה יוצא מעצמו מהבליעה
   של הגומי, שגדלה עם קצב העיוות — מכה חזקה בולעת הרבה וקפיצה
   קטנה כמעט לא.
   ...
   הזריקה הגדולה, כשהוא כבר נוסע. הוא יושב מחוץ לזרימה, ולכן רוחב
...
(function () {
  ...
  var YAW = -16 * Math.PI / 180;    // סטיית המצלמה: כך נחשפת רצועת הדריכה
  ...
  /* הסימן מתנהג כיריעת טרמפולינה: רך בהרבה מהצמיג, ולכן הוא זה
     שנמתח — הגלגל שוקע לתוכו ונזרק בחזרה. היריעה מתקשה ככל שהיא
     נמתחת, בדיוק כמו טרמפולינה אמיתית, וזה מה שמגביל את השקיעה. */
  var ML = 1.2, KL = 1400;
  /* היריעה בולעת אסימטרית: בדרך פנימה כמעט חופשית, ולכן היא נמתחת
     עמוק ורואים אותה עובדת; בדרך החוצה היא בולעת הרבה יותר, ולכן
     הזריקה יוצאת נמרצת אבל לא משגרת את הגלגל אל מחוץ למסך. ככה
     מתנהג גומי אמיתי — לולאת ההיסטרזיס שלו לא סימטרית. */
  var CL_IN = 8, CL_OUT = 95;
  var BED = 0.34;                         // מתיחה מרבית, בחלקי גובה הסימן
  ...
  var LEAN_KICK = 3.2e-4;                 // כמה כל נחיתה מנערת אותו
  ...
  /* הגלגל לא נופל מת: הוא כבר מסתובב כשהוא נכנס למסך, כמו גלגל
     ...
     שהוא תופס את היריעה — בלעדיו הוא נוחת בלי סיבוב, מחליק בכל
     נגיעה ומאבד חצי מהמהירות בכל נחיתה. */
  var vx = 0, vy = 0, rot = 0, om = OM0;
  var md = 0, mdv = 0;                     // מתיחת היריעה ומהירותה
  ...
  function sub(h) {
    ...
    // היריעה: קפיץ מתקשה, בליעה אסימטרית
    var u = md / (BED * markH);
    var cl = mdv > 0 ? CL_IN : CL_OUT;
    mdv += ((onLogo ? Fn : 0) - KL * md * (1 + 1.6 * u * u) - cl * mdv) / ML * h;
    md += mdv * h;
    ...
    if (wordT < 0 && pen > 0 && pastLogo) {
      /* הנגיעה הראשונה ברצפה: הזריקה הגדולה מהיריעה נגמרה, מכאן
  ...
  function paint() {
    ...
    var k = Math.max(-0.12, Math.min(0.40, md / markH));
